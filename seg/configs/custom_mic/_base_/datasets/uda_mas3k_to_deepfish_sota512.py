# UDA dataset config rebuilt from train_sota_segformer_crop.py + dataset_ce2ch_crop.py
# - train: random hflip + random rotate + random crop/pad with random paste location
# - val/test: keep-ratio downscale only + center pad to fixed 512
# - padded labels use ignore index 255
...
source_train_pipeline = [
    ...
    dict(type='RandomRotate', prob=0.2, degree=15, pad_val=0, seg_pad_val=255),
    dict(
        type='RandomCropPadAtRandomLocation',
        crop_size=crop_size,
        pad_val=0,
        seg_pad_val=255),
